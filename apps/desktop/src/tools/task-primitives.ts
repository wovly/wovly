...
/**
 * Task context with persistent memory
 */
 ...
 */
export interface SaveVariableResult extends BaseToolResult {
 ...
 * get_variable result
 */
export interface GetVariableResult extends BaseToolResult {
  name?: string;
  value?: string | null;
  exists?: boolean;
}
 ...
 */
export interface CheckVariableResult extends BaseToolResult {
  name?: string;
  value?: string | null;
  exists?: boolean;
 ...
 * parse_time result
 */
export interface ParseTimeResult extends BaseToolResult {
  hour?: number;
  minute?: number;
  formatted_24h?: string;
  formatted_12h?: string;
...
/**
 * check_time_passed result
 */
export interface CheckTimePassedResult extends BaseToolResult {
  passed?: boolean;
 ...
 * is_new_day result
 */
export interface IsNewDayResult extends BaseToolResult {
  is_new_day?: boolean;
  current_date?: string;
  last_date?: string;
}
 ...
 */
export interface EvaluateConditionResult extends BaseToolResult {
  result?: boolean;
  expression?: string;
}
 ...
 * goto_step result
 */
export interface GotoStepResult extends BaseToolResult {
  action?: 'goto_step';
  step_number?: number;
  reason?: string | null;
}
 ...
 */
export interface CompleteTaskResult extends BaseToolResult {
 ...
 */
export interface FormatStringResult extends BaseToolResult {
  result?: string;
  formatted?: string;
 ...
 */
export interface IncrementCounterResult extends BaseToolResult {
  action?: 'save_variable';
  name?: string;
  value?: string;
  previous?: number;
 ...
 * log_event result
 */
export interface LogEventResult extends BaseToolResult {
  action?: 'log_event';
  timestamp?: string;
  level?: string;
  log_entry?: string;
 ...
 * notify_user result
 */
export interface NotifyUserResult extends BaseToolResult {
 ...
 */
export interface SendChatMessageResult extends BaseToolResult {
  sent?: boolean;
}

/**
 * ask_user_question result
 */
export interface AskUserQuestionResult extends BaseToolResult {
  action?: 'wait_for_user_input';
  question?: string;
  save_response_as?: string | null;
  options?: string[] | null;
 ...
 */
export interface WaitForReplyResult extends BaseToolResult {
...
export const taskPrimitiveTools: Tool[] = [
  // ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export async function executeTaskPrimitiveTool(
  toolName: string,
  ...
  try {
    switch (toolName) {
      ...
      case "save_variable": {
        ...
        const result: SaveVariableResult = {
          success: true,
          action: "save_variable",
          name: name as string,
          value: String(value),
      ...
      case "get_variable": {
        ...
        const value = taskContext.contextMemory?.[name as string] ?? null;
      ...
      case "parse_time": {
        ...
        const parsed = parseTimeString(time_string as string);
        if (!parsed) {
      ...
      case "check_time_passed": {
        ...
        const targetTotalMinutes = (target_hour as number) * 60 + (target_minute as number);

        const minutesPast = currentTotalMinutes - targetTotalMinutes;
        const passed = minutesPast >= 0;
        const withinWindow = passed && minutesPast <= (tolerance_minutes as number);
        ...
        return {
          ...
          tolerance_minutes: tolerance_minutes as number,
          message: passed
            ? (withinWindow
      ...
      case "evaluate_condition": {
        ...
        const leftNum = parseFloat(left as string);
        const rightNum = parseFloat(right as string);
      ...
      case "goto_step": {
        ...
        if (!step_number || (step_number as number) < 1) {
        ...
        return {
          success: true,
          action: "goto_step",
          step_number: step_number as number,
      ...
      case "format_string": {
        ...
        console.log(`[TaskPrimitive] Format string: "${(template as string).slice(0, 50)}..." -> "${result.slice(0, 100)}..."`);
        return {
          success: true,
          result,
          ...
          variables_used: Object.keys((variables as Record<string, unknown>) || {})
      ...
      case "log_event": {
        ...
        const logEntry = `[${timestamp}] [${(level as string).toUpperCase()}] ${message}`;
      ...
      case "notify_user": {
        ...
        const emoji = typeEmoji[type as string] || "ℹ️";
      ...
      case "send_chat_message": {
        ...
        if (mainWindow && !mainWindow.isDestroyed()) {
          ...
          console.log(`[TaskPrimitive] Send chat message: ${(message as string).slice(0, 100)}...`);
          return {
            success: true,
            message: message as string,
            sent: true
      ...
      case "ask_user_question": {
        ...
        if (options && Array.isArray(options) && options.length > 0) {
          formattedQuestion += `\n\n**Options:**\n${(options as string[]).map((opt, i) => `${i + 1}. ${opt}`).join('\n')}`;
        }
        ...
        if (mainWindow && !mainWindow.isDestroyed()) {
          ...
          return {
            ...
            save_response_as: (save_response_as as string) || null,
      ...
      case "wait_for_reply": {
        ...
        return {
          ...
          original_request: original_request as string,
          success_criteria: success_criteria as string,
          conversation_id: (conversation_id as string) || null,
          poll_interval_minutes: poll_interval_minutes as number,
          followup_after_hours: followup_after_hours as number,
          max_followups: max_followups as number,
    ...
    console.error(`[TaskPrimitive] Error executing ${toolName}:`, (err as Error).message);
 ...
 */
export function parseTimeString(timeString: string): TimeParseResult | null {
 ...
 */
function formatTimeResult(hour: number, minute: number): TimeParseResult {
