...
export class BrowserController {
  ...
  async snapshot(sessionId: string = "default"): Promise<SnapshotResult> {
    ...
    try {
      ...
      console.error("[BrowserController] Failed to get accessibility tree:", (err as Error).message);
    }
  ...
  async click(sessionId: string, ref: string): Promise<ClickResult> {
    ...
    try {
      const clicked = await page.evaluate((name: string, role: string) => {
        ...
        for (const el of elements) {
          ...
          const keywordMatch = keywords.length > 0 && keywords.every((kw: string) =>
            allText.some((t: string) => t.includes(kw))
          );
    ...
    try {
      ...
      const clicked = await page.evaluate((role: string, targetIndex: number) => {
        let selector = 'button, a, input, select, textarea, [role="button"], [role="link"], [role="tab"], [role="menuitem"]';
        if (role === 'textbox') {
  ...
  async type(sessionId: string, text: string, ref: string | null = null, sensitive: boolean = false): Promise<TypeResult> {
  ...
  async scroll(sessionId: string, direction: 'up' | 'down' = 'down', amount: number = 500): Promise<ScrollResult> {
    const page = await this.getPage(sessionId);
