...
import { loadAllSkills, findBestSkill } from "../storage/skills";
import type { Skill } from "../storage/skills";

/**
 * Poll frequency preset types
 */
export type PollFrequencyPresetType = "preset" | "event";

/**
 * Poll frequency configuration
 */
export interface PollFrequency {
  type: PollFrequencyPresetType;
  value: number | string;
  label: string;
}

/**
 * Poll frequency presets map
 */
export const POLL_FREQUENCY_PRESETS: Record<string, PollFrequency> = {
...
export const DEFAULT_POLL_FREQUENCY: PollFrequency = { type: "preset", value: 60000, label: "Every 1 minute" };

/**
 * Task status
 */
export type TaskStatus = "pending" | "active" | "waiting" | "waiting_for_input" | "completed" | "cancelled";

/**
 * Task type
 */
export type TaskType = "discrete" | "continuous";
...
/**
 * Structured plan step (for direct execution)
 */
...
export interface Task {
  ...
  plan: string[];
  structuredPlan: StructuredPlanStep[] | null;
  currentStep: CurrentStep;
  executionLog: ExecutionLogEntry[];
  contextMemory: Record<string, string>;
  pendingMessages: PendingMessage[];
  _structuredPlanBuffer?: string; // Temporary parsing buffer
}
...
export interface CreateTaskData {
  ...
  originalRequest?: string;
  plan?: string[];
  structuredPlan?: StructuredPlanStep[];
  taskType?: TaskType;
  task_type?: TaskType;
...
/**
 * Get tasks directory for a user
 */
export const getTasksDir = async (username: string): Promise<string> => {
 ...
 */
export const parseTaskMarkdown = (markdown: string, taskId: string): Task => {
  const task: Task = {
    id: taskId,
    title: "",
    status: "pending",
  ...
  let currentPendingMessage: Partial<PendingMessage> | null = null;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("## ")) {
      if (currentPendingMessage && currentSection === "pending messages") {
        task.pendingMessages.push(currentPendingMessage as PendingMessage);
      ...
      if (match) {
        ...
        switch (key) {
          ...
          case "poll frequency": {
            ...
            if (parts.length >= 3) {
              task.pollFrequency = {
                type: parts[0] as PollFrequencyPresetType,
    ...
    if (currentSection === "pending messages") {
      if (line.startsWith("### Message")) {
        ...
        if (fieldMatch) {
          ...
          currentPendingMessage.message = (currentPendingMessage.message || '') + (currentPendingMessage.message ? "\n" : "") + line;
        }
      }
    }
  }
  ...
  if (task._structuredPlanBuffer) {
    try {
      ...
      if (jsonMatch) {
        task.structuredPlan = JSON.parse(jsonMatch[0]) as StructuredPlanStep[];
      }
    } catch (e) {
      console.error(`[Tasks] Failed to parse structured plan for ${taskId}:`, (e as Error).message);
    }
 ...
 * Create a new task
 */
export const createTask = async (taskData: CreateTaskData, username: string): Promise<Task> => {
  ...
  let skillContext: Record<string, string> = {};
  ...
  try {
    ...
    console.error(`[Tasks] Error matching skill:`, (err as Error).message);
  }

  const taskType = taskData.taskType || taskData.task_type || "discrete";

  let pollFrequency: PollFrequency = { ...DEFAULT_POLL_FREQUENCY };
  ...
  const task: Task = {
    ...
    contextMemory: {
      ...
      ...(taskType === "continuous" ? {
        ...
        monitoring_condition: taskData.monitoringCondition || taskData.monitoring_condition || '',
        trigger_action: taskData.triggerAction || taskData.trigger_action || ''
      } : {
        task_type: "discrete",
        success_criteria: taskData.successCriteria || taskData.success_criteria || ''
      })
    },
    hidden: false,
    autoSend: false,
 ...
 */
export const getTask = async (taskId: string, username: string): Promise<Task | null> => {
  const tasksDir = await getTasksDir(username);
  const taskPath = path.join(tasksDir, `${taskId}.md`);
  ...
  try {
    ...
    console.error(`[Tasks] Failed to read task ${taskId}:`, (err as Error).message);
    return null;
  }
};
 ...
 * List all tasks
 */
export const listTasks = async (username: string): Promise<Task[]> => {
  const tasksDir = await getTasksDir(username);
  ...
  try {
    ...
    const visibleTasks = tasks.filter((t: Task) => !t.hidden);
    visibleTasks.sort((a: Task, b: Task) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime());
    return visibleTasks;
  } catch (err) {
    console.error("[Tasks] Failed to list tasks:", (err as Error).message);
    return [];
  }
};
 ...
 */
export const listActiveTasks = async (username: string): Promise<Task[]> => {
  const tasks = await listTasks(username);
  return tasks.filter((t: Task) => t.status === "active" || t.status === "waiting");
};

/**
 * Get tasks waiting for user input
 */
export const getTasksWaitingForInput = async (username: string): Promise<Task[]> => {
  const tasks = await listTasks(username);
  return tasks.filter((t: Task) => t.status === "waiting_for_input");
};

/**
 * Cancel a task
 */
export const cancelTask = async (taskId: string, username: string): Promise<{ success?: boolean; error?: string }> => {
  const task = await getTask(taskId, username);
  if (!task) {
    return { error: `Task ${taskId} not found` };
  }
 ...
 * Hide a task
 */
export const hideTask = async (taskId: string, username: string): Promise<{ success?: boolean; error?: string }> => {
  const task = await getTask(taskId, username);
  if (!task) {
    return { error: `Task ${taskId} not found` };
  }
 ...
 */
export const getTaskRawMarkdown = async (taskId: string, username: string): Promise<string | { error: string }> => {
  const tasksDir = await getTasksDir(username);
  const taskPath = path.join(tasksDir, `${taskId}.md`);
 ...
 */
export const saveTaskRawMarkdown = async (
  taskId: string,
  markdown: string,
  username: string
  ...
  try {
    ...
    return { error: `Failed to save: ${(err as Error).message}` };
  }
};
