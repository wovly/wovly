/**
 * Asana Service
 * Handles Asana OAuth authentication
 */
...
export class AsanaService {
  ...
  static async checkAuth(
    ...
    try {
      ...
      const error = err instanceof Error ? err.message : 'Unknown error';
      console.error('[Asana] Error checking auth:', error);
      return { ok: false, error };
    }
  }

  static async disconnect(username: string | null | undefined): Promise<AsanaResponse> {
    try {
      if (!username) {
        return { ok: false, error: 'Not logged in' };
      }
      ...
      const error = err instanceof Error ? err.message : 'Unknown error';
      console.error('[Asana] Error disconnecting:', error);
      return { ok: false, error };
    }
  }
  ...
  static async test(
    ...
    try {
      ...
      const error = err instanceof Error ? err.message : 'Unknown error';
      console.error('[Asana] Error testing connection:', error);
      return { ok: false, error };
    }
  }
}
